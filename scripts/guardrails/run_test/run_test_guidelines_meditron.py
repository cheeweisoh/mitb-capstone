...
def generate_answer_meditron(model, tokenizer, question, guardrail_category, retries=2):
...
def main():
    ...
    for _, row in tqdm(qa_df.iterrows(), total=len(qa_df), desc="Generating Answers"):
        ...
        a = generate_answer_meditron(model, tokenizer, row["question"], guardrail_category, retries=2)
