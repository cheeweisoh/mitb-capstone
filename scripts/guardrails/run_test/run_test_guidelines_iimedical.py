...
def classify_guardrail_category(model, tokenizer, question):
    messages = [
        {"role": "user", "content": [{"type": "text", "text": guardrail_category_prompt(question)}]},
    ]
    ...
    with torch.inference_mode():
        raw = model.generate(**inputs, max_new_tokens=20, do_sample=False, pad_token_id=tokenizer.eos_token_id)
    text = tokenizer.decode(raw[0][input_len:], skip_special_tokens=True)
    return normalize_guardrail_categories(text)


def generate_answer_iimedical(model, tokenizer, question, guardrail_category, retries):
...
def main():
    ...
    for _, row in tqdm(qa_df.iterrows(), total=len(qa_df), desc="Generating Answers"):
        ...
        a = generate_answer_iimedical(model, tokenizer, row["question"], guardrail_category, 3)
