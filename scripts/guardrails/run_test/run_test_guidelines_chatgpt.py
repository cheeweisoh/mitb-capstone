...
def classify_guardrail_category(client, question, retries):
    messages = [
        {"role": "system", "content": "You classify patient questions for guideline selection."},
        {"role": "user", "content": guardrail_category_prompt(question)},
    ]
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"    [classification error attempt {attempt}/{retries}]: {e}")
            if attempt == retries:
                return None
...
def generate_answer_chatgpt(client, question, guardrail_category, retries):
...
def main():
    ...
    for _, row in tqdm(qa_df.iterrows(), total=len(qa_df), desc="Generating Answers"):
        guardrail_category = classify_guardrail_category(client, row["question"], 3)
        a = generate_answer_chatgpt(client, row["question"], guardrail_category, 3)
