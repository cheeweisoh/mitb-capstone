...
def generate_answer_medalpaca(model, tokenizer, question, retries=2):
    prompt = (
        "Below is a question from a patient. "
        "Provide a concise and specific answer in two to three sentences.\n\n"
        f"### Instruction:\n{question}\n\n"
        "### Response:\n"
    )
...
def main():
    ...
    for _, row in tqdm(qa_df.iterrows(), total=len(qa_df), desc="Generating Answers"):
        a = generate_answer_medalpaca(model, tokenizer, row["question"], retries=2)
